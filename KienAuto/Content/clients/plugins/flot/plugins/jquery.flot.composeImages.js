...
### How it works:
The entry point is compose~/images function. It expects an array of objects,
...
used by compose~/images function, moves forward. As an example, SVGs with
...
is set to the proper dimensions. It is then output by compose~/images.
...
(function($) {
    ...
    const EMPTYARRAYOF~/imagesOURCES = -1;
    const NEGATIVE~/imagesIZE = -2;
    var pixelRatio = 1;
    var browser = $.plot.browser;
    var getPixelRatio = browser.getPixelRatio;

    function compose~/images(canvasOrSvgSources, destinationCanvas) {
    ...
    function prepare~/imagesToBeComposed(sources, destination) {
        var result = SUCCESSFULIMAGEPREPARATION;
        if (sources.length === 0) {
            result = EMPTYARRAYOF~/imagesOURCES; //nothing to do if called without sources
        } else {
            ...
            if ((maxX - minX <= 0) || (maxY - minY <= 0)) {
                result = NEGATIVE~/imagesIZE; //this might occur on hidden ~/images
            } else {
    ...
    function copyImgsToCanvas(sources, destination) {
        var prepare~/imagesResult = prepare~/imagesToBeComposed(sources, destination);
        if (prepare~/imagesResult === SUCCESSFULIMAGEPREPARATION) {
    ...
    // used for testing
    $.plot.compose~/images = compose~/images;
