...
var Editor_Editor = /*#__PURE__*/function () {
  ...
  Editor_createClass(Editor, [{
    ...
    */
    ...
    key: "insert~/imagesAsDataURL",
    value: function insert~/imagesAsDataURL(files) {
      var _this4 = this;
    ...
    /**
     * insert~/imagesOrCallback
     * @param {File[]} files
     */

  }, {
    key: "insert~/imagesOrCallback",
    value: function insert~/imagesOrCallback(files) {
      var callbacks = this.options.callbacks; // If onImageUpload set,

      if (callbacks.onImageUpload) {
        this.context.triggerEvent('image.upload', files); // else insert Image as dataURL
      } else {
        this.insert~/imagesAsDataURL(files);
      }
    }
    /**
    ...
    value: function pasteByEvent(event) {
      ...
      if (clipboardData && clipboardData.items && clipboardData.items.length) {
        ...
        if (item.kind === 'file' && item.type.indexOf('image/') !== -1) {
          // paste img file
          this.context.invoke('editor.insert~/imagesOrCallback', [item.getAsFile()]);
          event.preventDefault();
    ...
    value: function attachDragAndDropEvent() {
      ...
      this.$dropzone.on('drop', function (event) {
        ...
        if (dataTransfer && dataTransfer.files && dataTransfer.files.length) {
          ...
          _this.context.invoke('editor.insert~/imagesOrCallback', dataTransfer.files);
        } else {
    ...
    value: function show() {
      ...
      this.showImageDialog().then(function (data) {
        ...
        if (typeof data === 'string') {
          ...
          // array of files
          _this.context.invoke('editor.insert~/imagesOrCallback', data);
        }
      }).fail(function () {
        _this.context.invoke('editor.restoreRange');
      });
